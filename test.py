...
class Worker():
    def __init__(self,car_name,s_size,a_size,trainer,model_path,global_episodes):
        ...
        self.summary_writer = tf.summary.FileWriter("train_"+self.number)
        self.local_AC = AC_Network(s_size,a_size,self.name,trainer)
        self.update_local_ops = update_target_graph('global',self.name)        
        self.actions = self.actions = np.identity(a_size,dtype=bool).tolist()
        self.env = game
        print self.env, "size is ",s_size, "action size is ",a_size
        print "trainer is ",self.trainer,"model path is",self.model_path, "global_episodes are",self.global_episodes
    ...
    def work(self,max_episode_length,gamma,sess,coord,saver):
        ...
        # print max_episode_length,gamma,sess,coord,saver
        episode_count = sess.run(self.global_episodes)
        total_steps = 0
        print ("Starting worker " + self.number)
